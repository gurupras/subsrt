...
class Subsrt {
  constructor () {
    this.format = {
      [vttHandler.name]: vttHandler,
      [lrcHandler.name]: lrcHandler,
      [smiHandler.name]: smiHandler,
      [ssaHandler.name]: ssaHandler,
      [assHandler.name]: assHandler,
      [subHandler.name]: subHandler,
      [srtHandler.name]: srtHandler,
      [sbvHandler.name]: sbvHandler,
      [jsonHandler.name]: jsonHandler
    }
  }
  ...
  convert (content, options) {
    ...
    let captions = this.parse(content, opt)
    if (opt.resync) {
      captions = this.resync(captions, opt.resync)
    }

    opt.format = opt.to || options.format
    const result = this.build(captions, opt)
